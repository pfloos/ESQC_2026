...
\begin{frame}{Expression of the HF energy }
	...
	\begin{block}{Solution}
		\begin{equation*}
			\begin{split}
				E_\text{HF} 
				& = \sum_i^N h_{ii} + \frac{1}{2} \sum_{ij}^N (\cJ_{ij} - \cK_{ij}) \quad \alert{\text{(cf few slides ago)}}
				\\
...
\begin{frame}{Unrestricted Hartree-Fock equations}
	\begin{block}{UHF equations for unrestricted spin orbitals}
		...
		\begin{align}
			& \boxed{
			\orange{f^\alpha(\br_1) \, \psi_i^\alpha(\br_1) = \epsilon_i^\alpha \, \psi_j^\alpha(\br_1)}
			}
			& 
			& \boxed{
			\alert{f^\beta(\br_1) \, \psi_i^\beta(\br_1) = \epsilon_i^\beta \, \psi_j^\beta(\br_1)}
			}
		\end{align}
		where
		\begin{align}
