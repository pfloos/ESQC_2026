	...
	\begin{columns}
		\begin{column}{0.4\textwidth}
			\begin{block}{In atomic units ($m = e = \hbar = 1$)}
				\begin{subequations}
				\begin{align}
					& \green{\chT_\text{n}} = - \sum_{A=1}^{M} \frac{\nabla_A^2}{2 M_A} 
					 \\	
					&  \blue{\chT_\text{e}} =  - \sum_{i=1}^{N} \frac{\nabla_i^2}{2} 
					\\
...
\begin{frame}{One-electron contributions to the HF energy}
	...
	\begin{block}{Nuclear repulsion}
			\begin{equation} 
				\mel{ \Psi_\text{HF} }{\purple{\chV_\text{nn}} }{ \Psi_\text{HF} } = \purple{V_\text{nn}} \braket{ \Psi_\text{HF} }{ \Psi_\text{HF} } = \purple{V_\text{nn}}
			\end{equation}
	\end{block}
...
\begin{frame}{Integral notations}
	\begin{block}{Spin orbitals}
		\begin{equation}
%			[i|h|j] = 
			\mel{i}{\hh}{j} = h_{ij} = \mel{\chi_i}{\hh}{\chi_j} = \int \dd{\bx_1} \chi_i^*(\bx_1) \hh(\br_1) \chi_j(\bx_1)
		\end{equation}
		\begin{equation}
			\braket{ij}{kl} 
...
%		\end{equation}
		\begin{equation}
			...
			= \int \dd{\bx_1} \int \dd{\bx_2} \chi_i^*(\bx_1) \chi_j^*(\bx_2) \frac{1}{r_{12}} \qty(1 - \underbrace{\chP_{12}}_{\text{permute 1 \& 2}}) \chi_k(\bx_1) \chi_l(\bx_2)
		\end{equation}
	\end{block}
	\begin{block}{Spatial orbitals}
		\begin{equation}
			(i|\hh|j) = h_{ij} = (\psi_i|\hh|\psi_j) = \int \dd{\br_1} \psi_i^*(\br_1) \hh(\br_1) \psi_j(\br_1) 
		\end{equation}
		\begin{equation}
			(ij|kl) 
...
\begin{frame}{One-electron energy of a two-electron determinant}
	...
	\begin{block}{Solution}
		...
		\begin{equation*}
		\begin{split}
			& \langle \Psi_\text{HF} | \hh(1) + \hh(2) | \Psi_\text{HF} \rangle 
			\\
			& \qquad = \frac{1}{2} \langle \chi_1(1) \chi_2(2) -  \chi_1(2) \chi_2(1) | \hh(1) + \hh(2) | \chi_1(1) \chi_2(2) -  \chi_1(2) \chi_2(1) \rangle
			\\
			&  \qquad = \frac{1}{2} \Big[ 
			\langle \chi_1(1) \chi_2(2)  | \hh(1) + \hh(2) | \chi_1(1) \chi_2(2)  \rangle
			- \langle \chi_1(1) \chi_2(2)  | \hh(1) + \hh(2) |  \chi_2(1) \chi_1(2) \rangle
			\\
			&  \qquad - \langle  \chi_2(1) \chi_1(2) | \hh(1) + \hh(2) | \chi_1(1) \chi_2(2)  \rangle
			+ \langle  \chi_2(1) \chi_1(2) | \hh(1) + \hh(2) |  \chi_2(1) \chi_1(2) \rangle
			\Big]
			\\
...
\begin{frame}{Integrating the one-electron spin coordinate}
	...
	\begin{block}{One-electron terms}
		...
		\begin{split}
			(\chi_1|\hh|\chi_1)
			& = \int \dd{\bx} \chi_1^*(\bx) \hh(\br) \chi_1(\bx)
			\\
			& = \int \dd{\omega} \int \dd{\br} \alpha^*(\omega) \psi_1^*(\br) \hh(\br) \alpha(\omega) \psi_1(\br)
			\\
			& = \underbrace{\qty[ \int \dd{\omega} \alpha^*(\omega) \alpha(\omega)]}_{=1} 
			\underbrace{\qty[ \int \dd{\br} \psi_1^*(\br) \hh(\br) \psi_1(\br) ]}_{(\psi_1|h|\psi_1)}
		\end{split}
		\end{equation*}
		\begin{equation*}
		\begin{split}
			(\chi_2|\hh|\chi_2) 
			& = \int \dd{\bx} \chi_2^*(\bx) \hh(\br) \chi_2(\bx) 
			\\
			& = \int \dd{\omega} \int \dd{\br} \beta^*(\omega) \psi_1^*(\br) \hh(\br) \beta(\omega) \psi_1(\br) 
			\\
...
\begin{frame}{Closed-shell Hartree-Fock energy in spatial orbitals}
	\begin{block}{General expression}
		\begin{equation}
			...
			= 2 \sum_i^{N/2} (i|\hh|i) + \sum_i^{N/2} \sum_j^{N/2} \qty[ 2(ii|jj) - (ij|ji) ]
		\end{equation}
	\end{block}
	\begin{block}{One- and two-electron terms}
		\small
		\begin{equation}
			\sum_i^N \mel{i}{\hh}{i} = \sum_i^{N/2} \mel{i}{\hh}{i} + \sum_a^{N/2} \mel{\Bar{i}}{\hh}{\Bar{i}} = 2 \sum_i^{N/2} (i|\hh|i)
		\end{equation}
		\begin{equation}
		\begin{split}
...
\begin{frame}{The Fock matrix}
	The \alert{variational principle} implies that the canonical orbitals diagonalize the \alert{one-electron Fock operator}
	\begin{equation*}
	...
	\end{equation*}	
	 ...
	 \begin{equation*}
		\hf(\br_1) = \hh(\br_1) + \sum_i^{N/2} [2 \hJ_i(\br_1) - \hK_i(\br_1)]	\quad \text{\alert{(closed shell)}}	
	\end{equation*}	
	The spin orbitals are the eigenfunctions of $\hf$:
	\begin{equation*}
		\boxed{\hf(\bx_1)\,\chi_i(\bx_1) = \epsilon_i \, \chi_i(\bx_1)}
	\end{equation*}	
	The eigenvalues $\epsilon_i$ are the \alert{orbital energies}.
\end{frame}
...
\begin{frame}{Hartree-Fock Hamiltonian}
	\begin{block}{Definition}
		\begin{equation}
			\underbrace{\chH^\text{HF}}_{\text{$N$-electron operator}} 
			= \sum_i^N \underbrace{\hf(\bx_i)}_{\text{one-electron operator}}
