...
\begin{frame}{The HF wave function}
	\begin{block}{A Slater determinant}
		...
		\end{equation}
		\begin{itemize}
			\item $\blue{\mathcal{A}}$ is called the \blue{antisymmetrizer}
...
\begin{frame}{Spin and spatial orbitals}
	...
	\begin{equation*}
		...
		\qquad
		\boxed{
		\blue{\psi_p(\br)} = \sum_\mu^K C_{\mu p} \green{\phi_{\mu}(\br)}
		}
	\end{equation*}
...
\begin{frame}{Restricted vs Unrestricted}
	...
	\begin{block}{Unrestricted Formalism}
		\begin{itemize}
			\item For open-shell systems we can use \alert{unrestricted HF (UHF)}
			\begin{equation*}
				\chi_p^\text{UHF}(\bx) = 
				\begin{cases}
				...
				\end{cases}
		\end{equation*}
			\item RHF = \green{Restricted} Hartree-Fock $\leftrightarrow$ \green{Roothaan-Hall equations}
			\item UHF = \orange{Unrestricted} Hartree-Fock $\leftrightarrow$ \orange{Pople-Nesbet equations}
			\item \blue{Restricted open-shell Hartree--Fock (ROHF)} retains shared spatial orbitals; we will not discuss it here
%			\item GHF = \red{Generalized} Hartree-Fock 
		\end{itemize}
	\end{block}
\end{frame}
...
\begin{frame}{RHF, ROHF and UHF}
	...
	\begin{itemize}
		\item RHF = \green{Restricted} Hartree-Fock
		\item UHF = \orange{Unrestricted} Hartree-Fock
...
\begin{frame}{The Hartree-Fock energy}
	...
	\begin{block}{One-electron Hamiltonian (or core Hamiltonian)}
		\begin{equation}
			\cO_1 = \blue{\cT_\text{e}} +  \orange{\cV_\text{ne}} = \sum_{i=1}^N h(\bx_i)	
			\qq{where}	
...
\begin{frame}{One-electron contributions to the HF energy}
	\begin{equation}
		\boxed{\cH_\text{elec} = \sum_{i=1}^N h(\bx_i) + \sum_{i<j}^N \frac{1}{r_{ij}}}
	\end{equation}	
	...
	\begin{block}{Core Hamiltonian:}
			\begin{equation} 
				\mel{ \Psi_\text{HF} }{ \cO_1 }{ \Psi_\text{HF} }  = \sum_{i=1}^N \mel{ \chi_i(\bx_1) }{ h(\bx_1) }{ \chi_i(\bx_1) } = \sum_{i=1}^N h_{ii}
			\end{equation}
	\end{block}
\end{frame}
...
\begin{frame}{Integral notations}
	\begin{block}{Spin orbitals}
		\begin{equation}
%			[i|h|j] = 
			\mel{i}{h}{j} = \int \dd{\bx_1} \chi_i^*(\bx_1) h(\br_1) \chi_j(\bx_1)
		\end{equation}
		\begin{equation}
			\braket{ij}{kl} 
...
%		\end{equation}
	...
	\begin{block}{Spatial orbitals}
		\begin{equation}
			(i|h|j) = h_{ij} = (\psi_i|h|\psi_j) = \int \dd{\br_1} \psi_i^*(\br_1) h(\br_1) \psi_j(\br_1) 
		\end{equation}
		\begin{equation}
			(ij|kl) 
...
\begin{frame}{Slater-Condon rules: One-electron operators}
	...
	\end{equation}
	\begin{block}{Case 1: determinants differ by zero spin orbitals; $\ket{K} = \ket{\cdots i j \cdots}$}
		\begin{equation}
			\mel{K}{\cO_1}{K} = \sum_i^N \mel{i}{h}{i}
		\end{equation}
	\end{block}
	\begin{block}{Case 2: determinants differ by one spin orbital; $\ket{K} = \ket{\cdots i j \cdots}$ and $\ket{L} = \ket{\cdots k j \cdots}$}
		\begin{equation}
			\mel{K}{\cO_1}{L} = \mel{i}{h}{k}
		\end{equation}
	\end{block}
	\begin{block}{Case 3: determinants differ by two spin orbitals; $\ket{K} = \ket{\cdots i j \ldots}$ and $\ket{L} = \ket{\cdots k l \ldots}$}
		\begin{equation}
			\mel{K}{\cO_1}{L} = 0
		\end{equation}
	\end{block}
...
\begin{frame}{Slater-Condon rules: Two-electron operators}
	...
	\begin{block}{Case 2: determinants differ by one spin orbital; $\ket{K} = \ket{\cdots i j \cdots}$ and $\ket{L} = \ket{\cdots k j \cdots}$}
		...
		\end{equation}
	\end{block}
	\begin{block}{Case 3: determinants differ by two spin orbitals; $\ket{K} = \ket{\cdots i j \cdots}$ and $\ket{L} = \ket{\cdots k l \cdots}$}
		\begin{equation}
...
\begin{frame}{Closed-shell HF energy in spatial orbitals}
...
\begin{frame}{The Fock matrix}
	The \alert{variational principle} implies that the canonical orbitals diagonalize the \blue{one-electron Fock operator}
	\begin{equation*}
		\boxed{ f(\bx_1) = h(\bx_1) + \underbrace{\sum_i^N [\cJ_i(\bx_1) - \cK_i(\bx_1)]}_{\nu^\text{HF}(\bx_1) \text{ = \blue{Hartree-Fock potential}}}}
	\end{equation*}	
	 For a \orange{closed-shell system} (i.e., two electrons in each spatial orbital)
	 \begin{equation*}
		f(\br_1) = h(\br_1) + \sum_i^{N/2} [2 J_i(\br_1) - K_i(\br_1)]	\quad \text{\alert{(closed shell)}}	
	\end{equation*}	
	The spin orbitals are the eigenfunctions of $f$:
	\begin{equation*}
	\boxed{f(\bx_1)\,\chi_i(\bx_1) = \epsilon_i \, \chi_i(\bx_1)}
	\end{equation*}	
	The eigenvalues $\epsilon_i$ are the \violet{orbital energies}.
\end{frame}
...
\begin{frame}{Hartree-Fock Hamiltonian}
	\begin{block}{Definition}
		\begin{equation}
			\underbrace{\cH^\text{HF}}_{\text{$N$-electron operator}} = \sum_i^N \underbrace{f(\bx_i)}_{\text{one-electron operator}}
		\end{equation}
	\end{block}
	\begin{block}{Warnings!}
		\begin{equation}
			\cH^\text{HF} \ket{\Psi_\text{HF}} = \qty( \sum_i^N \epsilon_i ) \ket{\Psi_\text{HF}}
...
\begin{frame}{The Roothaan-Hall matrix equations}
	...
	\begin{block}{Self-consistent field (SCF) procedure}
		\begin{equation}
			\bF(\bC) \cdot \bC 
			= \bS \cdot \bC \cdot \bE
			\qq{\alert{How do we solve these HF equations?}}
		\end{equation}
	\end{block}
\end{frame}
...
\begin{frame}{Expression of the Fock matrix}
	...
	\begin{equation*}
		\begin{split}
			F_{\mu \nu} 
			& = \mel{\mu}{h + \sum_i^N (\cJ_i - \cK_i)}{\nu} = H_{\mu \nu} +  \sum_i^N \mel{\mu}{\cJ_i - \cK_i}{\nu}
			\\
...
\begin{frame}{Expression of the HF energy}
	...
	\begin{block}{Solution}
		\begin{equation*}
			\begin{split}
				E_\text{HF} 
				& = \sum_i^N h_{ii} + \frac{1}{2} \sum_{ij}^N (\cJ_{ij} - \cK_{ij}) \quad \alert{\text{(as derived above)}}
				\\
				& = \sum_i^N  \mel{ \sum_\mu C_{\mu i} \phi_\mu }{ h }{  \sum_\nu C_{\nu i} \phi_\nu } 
...
\subsection{SCF}
%-----------------------------------------------------
\begin{frame}{How to perform an HF calculation in practice}
	\begin{block}{The SCF algorithm}
		\begin{enumerate}
			...
			\begin{enumerate}
				...
				\item[3.] Diagonalize $\bF'$ to obtain $\bC'$ and $\bE$
				\item[4.] Calculate $\bC= \bX \cdot  \bC'$
				\item[5.] Form a \blue{new density matrix} $\bP = 2\,\bC_{\mathrm{occ}} \cdot \bC_{\mathrm{occ}}^\dag$
				\item[6.] \alert{Am I converged?} If not go back to 1.
			\end{enumerate}
...
\begin{frame}{Orthogonalization matrix}
	\red{\bf We seek a matrix that orthogonalizes the AO basis, i.e., $\bX^\dag \cdot \bS \cdot  \bX = \bI$}
	\\
	\bigskip
	\begin{columns}
		\begin{column}{0.7\textwidth}
...
\begin{frame}{How to obtain a good guess for the MOs or density matrix?}
	\begin{block}{Possible initial density matrix}
		\begin{enumerate}
			\bigskip
			\item We can set \purple{$\bP = \bO$ $\Rightarrow$ $\bF = \bH$} (\orange{core Hamiltonian approximation}):\\
			$\Rightarrow$ Usually a poor guess but easy to implement
			\bigskip
			\item Use \alert{EHT or a semi-empirical method}:\\
			$\Rightarrow$ Inexpensive, though now less common
			\bigskip
			\item Use \violet{tabulated atomic densities}:\\
...
\begin{frame}{Dipole moments}
	...
	\begin{block}{Vector components}
		\begin{equation}
			...
			\qq{with}
			\underbrace{(\nu|x|\mu)}_{\text{one-electron integrals}} = \int \dd{\br} \phi_\nu^*(\br) \,x\, \phi_\mu(\br) 
		\end{equation}
	\end{block}
\end{frame}
...
\begin{frame}{Unrestricted HF (UHF)}
	\begin{block}{How to model open-shell systems?}
		\begin{itemize}
			\item For open-shell systems one uses \violet{unrestricted spin orbitals}
			\begin{equation*}
				\chi_p^\text{UHF}(\bx) = 
				\begin{cases}
					\alpha(\omega) \, \orange{\psi_p^\alpha(\br)}
					\\
					\beta(\omega) \, \red{\psi_p^\beta(\br)}
				\end{cases}
			\end{equation*}		
			\item UHF may break spin symmetry: the determinant need not be an eigenfunction of $\hat{S}^2$
			...
			\item Generalized HF also allows spin mixing and need not preserve $\hat{S}_z$
		\end{itemize}
	\end{block}
\end{frame}
...
\begin{frame}{Unrestricted Hartree--Fock equations}
	...
	\begin{block}{UHF equations for unrestricted spin orbitals}
		\bigskip
		\violet{To minimize the UHF energy}, the unrestricted spatial orbitals must be eigenfunctions of the \blue{$\alpha$ and $\beta$ Fock operators}:
		\begin{align}
			& \boxed{
		...
		\begin{align}
			\blue{J_i^\sigma(\br_1)} & = \int \dd{\br_2} \psi_i^{\sigma *}(\br_2) r_{12}^{-1} \psi_i^\sigma(\br_2) 
			& 
			\violet{K_i^\sigma(\br_1)} \psi_j^\sigma(\br_1) & =  \qty[ \int \dd{\br_2} \psi_i^{\sigma *}(\br_2) r_{12}^{-1} \psi_j^\sigma(\br_2) ] \psi_i^\sigma(\br_1)
		\end{align}		
	\end{block}
\end{frame}
	...
	\begin{block}{The SCF algorithm}
		\begin{enumerate}
			...
			\begin{enumerate}
				...
				\item[5.] Form the \blue{new density matrices} $\bP^\alpha$ and $\bP^\beta$, and compute $\bP^\text{T} = \bP^\alpha + \bP^\beta$
				\item[6.] \alert{Am I converged?} If not go back to 1.
			\end{enumerate}
