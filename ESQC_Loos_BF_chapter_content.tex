...
\red{Gaussian-type orbitals (GTOs)}, which are proportional to
...
\blue{Slater-type orbitals (STOs)} are also atom centered, but are
proportional to $\exp(-\zeta r)$. They possess the correct nuclear cusp and
long-range tail and are used, for example, in \textsc{ADF}. By contrast,
\orange{plane waves (PWs)}, proportional to $\exp(\ii\bk\cdot\br)$, are
...
\textsc{CASTEP}. Finally, \green{numerical atomic orbitals (NAOs)} have
...
flexibility.
\red{Radial flexibility} is provided by multiple-$\zeta$ functions, as in
double-$\zeta$, triple-$\zeta$, and quadruple-$\zeta$ bases.
\blue{Angular flexibility} is supplied by polarization functions, such as
$d$ functions on second-row atoms and $p$ functions on hydrogen.
\orange{Long-range flexibility} is supplied by diffuse functions with small
...
\green{Core flexibility}, in turn, is supplied by tight functions and is
...
Pople basis sets extend the minimal description by splitting the valence space
...
The correlation-consistent construction organizes functions into successive
...
Jensen's polarization-consistent sets apply systematic improvement to HF and
