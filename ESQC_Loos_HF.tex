...
\begin{frame}{Spatial orbitals}
	...
	\begin{block}{The same-spin unrestricted spatial orbitals are orthogonal}
		\begin{align}
			\braket{ \psi^{\red{\alpha}}_p }{ \psi^{\red{\alpha}}_q } & = \int \psi^{\red{\alpha}*}_p(\br) \psi^{\red{\alpha}}_q(\br) \dd{\br} = \delta_{pq} 
			\\ 
			\braket{ \psi^{\blue{\beta}}_p }{ \psi^{\blue{\beta}}_q } & = \int \psi^{\blue{\beta}*}_p(\br) \psi^{\blue{\beta}}_q(\br) \dd{\br} = \delta_{pq} 
		\end{align}
	\end{block}
	\begin{block}{The opposite-spin unrestricted spatial orbitals are, a priori, not orthogonal}
		\begin{equation}
			\braket{ \psi^{\red{\alpha}}_p }{ \psi^{\blue{\beta}}_q }  = \int \psi^{\red{\alpha}*}_p(\br) \psi^{\blue{\beta}}_q(\br) \dd{\br} = S_{pq}^{\red{\alpha}\blue{\beta}} 
		\end{equation}
	\end{block}
\end{frame}
...
\begin{frame}{Hartree-Fock Hamiltonian}
	...
	\begin{block}{Warnings!}
		...
		\begin{equation}
			\mel{\Psi_\text{HF}}{\chH}{\Psi_\text{HF}} \green{=} E_\text{HF}
		\end{equation}
...
\begin{frame}{Ordinary SCF tends to collapse to the ground state}
	An excited determinant can be generated by promoting an electron,
...
%				\node[hfRed,left] at (-0.75,0.72) {hole};
	...
	\begin{block}{The problem}
		After each diagonalization, Aufbau occupies the orbitals with the
		\alert{lowest energies}; the SCF therefore tends to return to a lower-energy solution.
	\end{block}
\end{frame}

%-----------------------------------------------------

\begin{frame}{MOM follows orbital character rather than orbital energy}
	Let $\bC_{\mathrm{o}}^\text{ref}$ contain the occupied orbitals that define
	the target configuration. For each new orbital $a$, compute
	\begin{align}
		O_{ia}
		&=
		\braket{\psi_i^\text{ref}}{\psi_a^\text{new}}
		=
		\sum_{\mu\nu}
		C_{\mu i}^{\text{ref}*}S_{\mu\nu}C_{\nu a}^\text{new},
		&
		p_a
		&=
		\sum_{i\in\mathrm{occ}(\mathrm{ref})}\abs{O_{ia}}^2 .
	\end{align}

	\begin{columns}[T,onlytextwidth]
		\begin{column}{0.50\textwidth}
			\begin{block}{Maximum overlap method (MOM)}
				\begin{itemize}
					\item Rank the new orbitals using $p_a$
					\item Occupy the $N$ orbitals with the largest projections
					\item In MOM, the reference is the occupied space from the
					previous SCF iteration
				\end{itemize}
			\end{block}
			\begin{block}{Initial MOM (IMOM)}
				The reference orbitals remain fixed to the
				\alert{initial excited-state guess}, which reduces gradual drift
				away from the targeted configuration.
			\end{block}
		\end{column}
		\begin{column}{0.42\textwidth}
			\centering
			\textbf{New orbitals after diagonalization}
			\begin{tikzpicture}[x=1cm,y=0.68cm]
				\draw[thick] (-1.2,0)--(1.2,0);
				\node[right] at (1.3,0) {$\epsilon_1$};
				\node[left,hfInk] at (-1.3,0) {$p_1=0.08$};
				\draw[thick,hfBlue] (-1.2,1.1)--(1.2,1.1);
				\node[right] at (1.3,1.1) {$\epsilon_2$};
				\node[left,hfBlue] at (-1.3,1.1) {$p_2=0.93$};
				\fill[hfBlue] (0,1.1) circle (0.09);
				\draw[thick] (-1.2,2.2)--(1.2,2.2);
				\node[right] at (1.3,2.2) {$\epsilon_3$};
				\node[left,hfInk] at (-1.3,2.2) {$p_3=0.12$};
				\draw[thick,hfBlue] (-1.2,3.3)--(1.2,3.3);
				\node[right] at (1.3,3.3) {$\epsilon_4$};
				\node[left,hfBlue] at (-1.3,3.3) {$p_4=0.86$};
				\fill[hfBlue] (0,3.3) circle (0.09);
			\end{tikzpicture}
			
			\alert{Occupy the largest projections: the selected orbitals need not be the lowest in energy.}
		\end{column}
	\end{columns}
\end{frame}
...
\begin{frame}{MOM changes only the occupation step of SCF}
	\centering
	\begin{tikzpicture}[
		...
		\node[box,right=of fock] (solve) {Solve\\$\bF\bC=\bS\bC\bE$};
		\node[box,right=of solve] (overlap) {Compute\\$O_{ia}$ and $p_a$};
		\node[box,right=of overlap] (occupy) {Occupy largest\\$p_a$ values};
	...
	\begin{columns}[T,onlytextwidth]
		\begin{column}{0.47\textwidth}
			\begin{block}{Conventional SCF}
				...
				\begin{equation}
					\epsilon_1\leq\epsilon_2\leq\cdots
				\end{equation}
		...
		\begin{column}{0.47\textwidth}
			\begin{block}{MOM-SCF}
				\centering
				Occupy the largest overlap scores
				\begin{equation}
					p_{a_1}\geq p_{a_2}\geq\cdots
				\end{equation}
	...
	\vfill
	\alert{The Fock build, diagonalization, DIIS machinery, and convergence tests are unchanged.}
\end{frame}
...
\begin{frame}{What does a MOM calculation give?}
	\begin{equation}
		\boxed{
			\omega_{\Delta\mathrm{SCF}}
			=
			E_\mathrm{HF}^{\mathrm{MOM}}
			-
			E_\mathrm{HF}^{\mathrm{GS}}
		}
	\end{equation}
		...
		\begin{column}{0.47\textwidth}
			\begin{block}{What is gained?}
				\begin{itemize}
					...
					\item A cost similar to one ground-state SCF calculation
				\end{itemize}
			\end{block}
		\end{column}
		\begin{column}{0.47\textwidth}
	...
	\vfill
	\centering
	$\Rightarrow$ \alert{MOM prevents variational collapse; it does not make HF an exact excited-state theory.}
