...
\begin{frame}{Bosons vs Fermions}
	\begin{block}{Symmetry vs Antisymmetry}
		...
		\begin{equation*}
			\boxed{\abs{\Psi(\bx_1,\bx_2)}^2 = \abs{\Psi(\bx_2,\bx_1)}^2  \qq{$\Rightarrow$}  \Psi(\bx_2,\bx_1) = \alert{e^{\ii \theta}} \Psi(\bx_1,\bx_2)}
		\end{equation*}
		Applying twice the \alert{permutation operator} $\chP_{12}$ gives
		\begin{gather*}
			\chP_{12} \Psi(\bx_1,\bx_2) = \Psi(\bx_2,\bx_1) = \alert{e^{\ii \theta}} \Psi(\bx_1,\bx_2)
			\qquad
			\chP_{12}^2 \Psi(\bx_1,\bx_2) = \alert{e^{2\ii \theta}} \Psi(\bx_1,\bx_2) = \Psi(\bx_1,\bx_2)
			\\
			\qq{$\Rightarrow$}
			\alert{e^{2\ii \theta}} = 1 
			\qq{$\Rightarrow$}
