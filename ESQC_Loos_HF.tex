	...
	\begin{columns}[T,onlytextwidth]
		\begin{column}{0.48\textwidth}
			\begin{block}{An excited determinant is}
				\begin{itemize}
					\item[\cmark] An orbital substitution within a \alert{fixed} set of reference orbitals
					\item[\cmark] A valid many-electron basis function
					\item[\cmark] A building block for CI and CC expansions
				\end{itemize}
			\end{block}
		\end{column}
		\begin{column}{0.48\textwidth}
...
\begin{frame}{DIIS take-home messages}
	\begin{itemize}
		...
		\bigskip
		\item[\cmark] A small \alert{rolling subspace} is normally sufficient: commonly $m\approx6$--$10$
		\bigskip
		\item[\xmark] DIIS requires additional \alert{storage}: $\sim 2mK^2$
		\bigskip
		\item[\xmark] The extrapolated energy is not guaranteed to decrease monotonically
		\bigskip
		\item[\xmark] $\bB$ can be \alert{singular} due to nearly linear dependence of the residuals: a restart may be required
		\bigskip
		...
		\begin{column}{0.48\textwidth}
			\begin{block}{What is gained?}
				\begin{itemize}
					...
					\bigskip
					\item[\cmark] Useful excitation energies for states dominated by a single electronic configuration
					\bigskip
					...
					\bigskip
					\item[\cmark] The same formal cost per SCF iteration, although convergence may be more difficult
				\end{itemize}
			\end{block}
		\end{column}
		\begin{column}{0.48\textwidth}
	...
	\begin{columns}[T,onlytextwidth]
		\begin{column}{0.48\textwidth}
			\begin{block}{Strengths}
				\begin{itemize}
					\item[\cmark] Requires only one-electron integrals
					\item[\cmark] Produces compact bonds and lone pairs
				\end{itemize}
			\end{block}
		\end{column}
		\begin{column}{0.48\textwidth}
			\begin{block}{Limitations}
				\begin{itemize}
					\item[\xmark] Does not explicitly preserve $\sigma/\pi$ separation
...
\begin{frame}{Pipek-Mezey localization procedure}
	...
	\begin{columns}[T,onlytextwidth]
		\begin{column}{0.48\textwidth}
			\begin{block}{Strengths}
				\begin{itemize}
					\item[\cmark] Often preserves separation between $\sigma$ and $\pi$ orbitals
					\item[\cmark] Produces intuitive bonds, lone pairs, and core orbitals
				\end{itemize}
			\end{block}
		\end{column}
		\begin{column}{0.48\textwidth}
			\begin{block}{Limitations}
				\begin{itemize}
					...
					\item[\xmark] L{\"o}wdin populations or intrinsic atomic orbitals provide more robust alternatives
				\end{itemize}
			\end{block}
		\end{column}
	\end{columns}
...
%\begin{frame}{Coulomb operator: density creates a local potential}
%	\begin{columns}[T]
...
%				}
%			]
%				\node[box] (rhoj) at (0,2.8)
%					{$\green{\rho_j(\bx_2)=\abs{\chi_j(\bx_2)}^2}$\\
%					 source charge density};
%
%				\node[box] (pot) at (0,1.25)
%					{$\blue{v_j^\text{J}(\br_1)}$\\
%					 electrostatic potential};
%
%				\node[box] (rhoi) at (-2.0,-0.45)
%					{$\violet{\rho_i(\bx_1)}$\\
%					 probe density};
%
%				\node[box] (energy) at (2.0,-0.45)
%					{$\orange{J_{ij}}$\\
%					 Coulomb energy};
%
%				\draw[->,very thick,hfBlue] (rhoj) -- (pot)
%					node[midway,right,text=hfInk] {$r_{12}^{-1}$};
%
%				\draw[->,very thick,hfOrange] (pot.south) -- ++(0,-0.35)
%					-| (energy.north);
%
%				\draw[->,very thick,hfViolet] (rhoi) -- (energy);
%			\end{tikzpicture}
%		\end{column}
%		%
%		\begin{column}{0.43\textwidth}
%			\begin{block}{Step 1: generate the potential}
%				\begin{equation}
%					\blue{v_j^\text{J}(\br_1)}
%					=
%					\int
%					\frac{\green{\rho_j(\bx_2)}}{r_{12}}
%					\dd{\bx_2}
%				\end{equation}
%			\end{block}
%			\begin{block}{Step 2: probe the potential}
%				\begin{equation}
%					\orange{J_{ij}}
%					=
%					\int
...
%	\begin{equation}
%		\boxed{
%			\hJ_j(\bx_1)\chi_i(\bx_1)
%			=
%			v_j^\text{J}(\br_1)\chi_i(\bx_1)
%		}
%		\qquad
%		\Rightarrow
%		\qquad
%		\alert{$\hJ_j$ is a local multiplicative operator}
%	\end{equation}
%\end{frame}

\begin{frame}{Exchange operator: a nonlocal quantum effect}
	\begin{columns}[T]
		\begin{column}{0.52\textwidth}
			\begin{block}{Coulomb operator}
				...
				The input orbital remains $\chi_i$:
				\[
					\hJ_j\chi_i=v_j^\text{J}\chi_i.
				\]
			\end{block}
		\end{column}
		%
		\begin{column}{0.48\textwidth}
			\begin{block}{Exchange operator}
				\centering
				\begin{tikzpicture}[
					>=stealth,
					box/.style={
					...
					\node[box] (in) at (0,2.0)
						{$\chi_i(\bx_2)$};
					\node[box] (op) at (0,0.8)
						{$\violet{\chi_j^*(\bx_2)\chi_i(\bx_2)}$};
					\node[box] (out) at (0,-0.4)
						{$k_{ji}(\br_1)\chi_j(\bx_1)$};

					\draw[->,thick] (in) -- (op);
					\draw[->,thick] (op) -- (out);
				\end{tikzpicture}

				The output is proportional to $\chi_j$:
				\[
					\hK_j\chi_i
					=
					\left[
					\int
					\frac{\chi_j^*(\bx_2)\chi_i(\bx_2)}
					     {r_{12}}
					\dd{\bx_2}
					\right]\chi_j(\bx_1).
				\]
	...
	\begin{center}
		\alert{Exchange has no classical electrostatic analogue:
		it is orbital dependent and nonlocal.}
	\end{center}
\end{frame}

%\begin{frame}{The self-consistent-field cycle}
%	\centering
%	\begin{tikzpicture}[
%		>=stealth,
...
%		\node[step,right=of mol] (ints)
%			{Compute\\integrals};
%		\node[step,right=of ints] (guess)
%			{Initial density\\$\bP^{(0)}$};
%		\node[step,right=of guess] (fock)
%			{Build Fock matrix\\$\bF[\bP^{(n)}]$};
%
%		\node[step,below=1.15cm of fock] (solve)
%			{Solve\\$\bF\bC=\bS\bC\bE$};
%		\node[step,left=of solve] (density)
%			{New density\\$\bP^{(n+1)}$};
%		\node[decision,left=of density] (conv)
%			{Converged?};
%		\node[step,left=of conv] (properties)
...
%		\draw[->,thick,hfGreen] (conv) -- (properties)
...
%			{optional damping or DIIS};
%	\end{tikzpicture}
%
%	\begin{block}{Self-consistency}
%		\begin{equation}
%			\boxed{
%				\bP^{(n)}
%				\longrightarrow
...
\begin{frame}{DIIS searches for a nearly vanishing residual}
	\begin{columns}[T]
		\begin{column}{0.53\textwidth}
			\begin{block}{Residual space}
				\centering
				\begin{tikzpicture}[scale=0.83,>=stealth]
					\draw[->] (-0.3,0) -- (5.0,0);
					\draw[->] (0,-0.3) -- (0,4.1);
					\node[below left] at (0,0) {$\bO$};

					\draw[dashed,hfInk,thick]
						(0.7,3.6) -- (4.8,1.0)
						node[right] {affine subspace};

					\fill[hfBlue] (1.1,3.35) circle (2.5pt)
						node[above right] {$\be^{(1)}$};
					\fill[hfOrange] (2.7,2.34) circle (2.5pt)
						node[above right] {$\be^{(2)}$};
					\fill[hfViolet] (4.25,1.35) circle (2.5pt)
						node[above right] {$\be^{(3)}$};

					\fill[hfRed] (1.72,2.95) circle (3pt)
						node[above right] {$\be_\text{DIIS}$};

					\draw[dotted,very thick,hfRed]
						(0,0) -- (1.72,2.95);

					\node[align=center,text=hfRed] at (3.5,3.55)
						{closest accessible\\point to zero};
				\end{tikzpicture}
			\end{block}
		\end{column}
		%
		\begin{column}{0.47\textwidth}
			...
			\begin{block}{Why the constraint?}
				$\sum_i c_i=1$ defines an
				\alert{affine combination} of the stored iterations,
				rather than an arbitrary rescaling toward zero.
			\end{block}
		\end{column}
	\end{columns}

	\begin{center}
		\green{DIIS combines previous Fock matrices so that their
		combined SCF residual is as small as possible.}
	\end{center}
\end{frame}

\begin{frame}{Spin contamination as a mixture of spin states}
	\begin{columns}[T]
		\begin{column}{0.53\textwidth}
			\begin{block}{A UHF determinant is generally not spin pure}
				\begin{equation}
					\ket{\Phi_\text{UHF}}
					=
					c_S\ket{S,M_S}
					...
					+\cdots
				\end{equation}
				The components have the same $M_S$, but different
				eigenvalues of $\chS^2$.
			\end{block}

			\begin{equation}
				\expval{\chS^2}_\text{UHF}
				=
				\sum_{S'}\abs{c_{S'}}^2S'(S'+1)
			\end{equation}
		\end{column}
		%
		\begin{column}{0.47\textwidth}
			\begin{block}{Schematic composition}
				\centering
				\begin{tikzpicture}
					\draw[fill=hfBlue!75,draw=none]
						(0,0) rectangle (2.2,3.9);
					\draw[fill=hfOrange!80,draw=none]
						(0,3.9) rectangle (2.2,4.8);
					\draw[fill=hfViolet!80,draw=none]
						(0,4.8) rectangle (2.2,5.35);

					\draw[hfInk,thick] (0,0) rectangle (2.2,5.35);

					\node[white,align=center] at (1.1,1.95)
						{target\\$\ket{S,M_S}$};
					\node[white] at (1.1,4.35)
						{$S+1$};
					\node[white] at (1.1,5.07)
						{$S+2,\ldots$};

					\node[right,align=left] at (2.45,2.0)
						{desired\\component};
					\node[right,align=left,text=hfRed] at (2.45,4.7)
						{spin\\contamination};
				\end{tikzpicture}
			\end{block}
		\end{column}
	...
	\begin{equation}
		...
		\qquad
		\text{for a target state with $S=M_S$.}
	\end{equation}
\end{frame}

\begin{frame}{Molecular orbitals are built from atomic basis functions}
	\begin{block}{Constructive and destructive combinations}
		\centering
		\begin{tikzpicture}[scale=0.9,>=stealth]
			% Bonding row
			\node at (-0.4,2.1) {\green{in phase}};
			\fill[hfBlue!70] (1.1,2.1) ellipse (0.65 and 0.34);
			\fill[hfBlue!70] (2.5,2.1) ellipse (0.65 and 0.34);
			\node[white] at (1.1,2.1) {$+$};
			\node[white] at (2.5,2.1) {$+$};
			\fill[hfInk] (1.55,2.1) circle (1.7pt);
			\fill[hfInk] (2.05,2.1) circle (1.7pt);

			\draw[->,very thick] (3.3,2.1) -- (4.3,2.1);

			\fill[hfBlue!60] (5.8,2.1) ellipse (1.55 and 0.48);
			\fill[hfInk] (5.55,2.1) circle (1.7pt);
			\fill[hfInk] (6.05,2.1) circle (1.7pt);
			\node at (7.65,2.1) {$\psi_\text{bonding}$};

			% Antibonding row
			\node at (-0.4,0.2) {\red{out of phase}};
			\fill[hfBlue!70] (1.1,0.2) ellipse (0.65 and 0.34);
			\fill[hfRed!70]  (2.5,0.2) ellipse (0.65 and 0.34);
			\node[white] at (1.1,0.2) {$+$};
			\node[white] at (2.5,0.2) {$-$};
			\fill[hfInk] (1.55,0.2) circle (1.7pt);
			\fill[hfInk] (2.05,0.2) circle (1.7pt);

			\draw[->,very thick] (3.3,0.2) -- (4.3,0.2);

			\fill[hfBlue!70] (5.15,0.2) ellipse (0.72 and 0.42);
			\fill[hfRed!70]  (6.45,0.2) ellipse (0.72 and 0.42);
			\draw[white,very thick] (5.8,-0.35) -- (5.8,0.75);
			\fill[hfInk] (5.55,0.2) circle (1.7pt);
			\fill[hfInk] (6.05,0.2) circle (1.7pt);
			\node at (7.75,0.2) {$\psi_\text{antibonding}$};
		\end{tikzpicture}
	\end{block}
	...
	\begin{equation}
		\boxed{
			\psi_p(\br)
			=
			\sum_{\mu=1}^{K} C_{\mu p}\phi_\mu(\br)
		}
	\end{equation}

	\begin{itemize}
		\item The coefficients determine the \alert{weight and phase}
			of each basis function.
		\item Constructive overlap increases density between the nuclei;
			destructive overlap produces a node.
	\end{itemize}
\end{frame}

\begin{frame}{The dipole moment measures charge separation}
	\begin{columns}[T]
		\begin{column}{0.57\textwidth}
			\begin{block}{Electronic and nuclear charge centers}
				\centering
				\begin{tikzpicture}[>=stealth,scale=0.9]
					% Electron cloud
					\fill[hfBlue!18]
						(0,0) ellipse (3.2 and 1.55);
					\fill[hfBlue!16]
						(-1.1,0.2) ellipse (1.7 and 1.05);
					\fill[hfBlue!16]
						(1.45,-0.15) ellipse (1.45 and 0.95);

					% Nuclei
					\fill[hfOrange] (-1.5,0.15) circle (5pt);
					\fill[hfOrange] (0.8,0.45) circle (4pt);
					\fill[hfOrange] (1.75,-0.45) circle (4pt);

					\node[above,text=hfOrange] at (-1.5,0.3) {$Z_A$};
					\node[above,text=hfOrange] at (0.8,0.6) {$Z_B$};
					\node[below,text=hfOrange] at (1.75,-0.6) {$Z_C$};

					% Charge centers
					\fill[hfBlue] (-0.45,-0.25) circle (3pt);
					\node[below,text=hfBlue] at (-0.45,-0.35)
						{$\bR_-$};

					\fill[hfRed] (0.45,0.15) circle (3pt);
					\node[above,text=hfRed] at (0.45,0.25)
						{$\bR_+$};

					\draw[->,very thick,hfViolet]
						(-0.45,-0.25) -- (0.45,0.15)
						node[midway,above] {$\boldsymbol{\mu}$};

					\node[text=hfBlue] at (0,-1.9)
						{electronic density};
				\end{tikzpicture}
			\end{block}
		\end{column}
		%
		\begin{column}{0.43\textwidth}
			\begin{block}{Quantum-mechanical expression}
				\begin{equation}
					\boldsymbol{\mu}
					=
					-\int
...
\begin{frame}{Population analysis partitions an overlapping basis}
	\begin{columns}[T]
		\begin{column}{0.49\textwidth}
			\begin{block}{Mulliken population analysis}
				...
				\begin{tikzpicture}[scale=0.9]
					\fill[hfBlue!35]
						(-0.75,0) ellipse (1.6 and 0.9);
					\fill[hfOrange!35]
						(0.75,0) ellipse (1.6 and 0.9);

					\fill[hfBlue] (-1.25,0) circle (3pt);
					\fill[hfOrange] (1.25,0) circle (3pt);

					\node[below] at (-1.25,-0.15) {$A$};
					\node[below] at (1.25,-0.15) {$B$};
					\node at (0,0.25) {overlap};
					\node at (0,-1.25)
						{$S_{\mu\nu}\neq0$};
				\end{tikzpicture}
				...
				The overlap population is assigned directly
				in the original AO basis.
			\end{block}
		\end{column}
		%
		\begin{column}{0.51\textwidth}
			\begin{block}{Löwdin population analysis}
				\centering
				\begin{tikzpicture}[scale=0.9,>=stealth]
					\fill[hfBlue!35]
						(-1.7,0) ellipse (1.15 and 0.85);
					\fill[hfOrange!35]
						(1.7,0) ellipse (1.15 and 0.85);

					\fill[hfBlue] (-1.7,0) circle (3pt);
					\fill[hfOrange] (1.7,0) circle (3pt);

					\node[below] at (-1.7,-0.15) {$A$};
					\node[below] at (1.7,-0.15) {$B$};
					\node at (0,-1.25)
						{$\widetilde{\bS}=\bI$};
				\end{tikzpicture}
				...
				\[
					...
					q_A^\text{Löwdin}
					=
					Z_A-\sum_{\mu\in A}\widetilde P_{\mu\mu}.
				\]

				The basis is symmetrically orthogonalized
				before populations are assigned.
			\end{block}
		\end{column}
	\end{columns}

	\begin{center}
		\alert{Atomic charges are not observables:
		they depend on the chosen partitioning scheme.}
	\end{center}
\end{frame}

\begin{frame}{Charge and spin densities contain different information}
	\centering
	\begin{tikzpicture}[
		>=stealth,
		panel/.style={
		...
		\node[panel] (a) at (-4.2,1.25) {
			\blue{$\rho^\alpha(\br)$}\\[0.4em]
			\begin{tikzpicture}
				\fill[hfBlue!65] (-0.45,0) circle (0.42);
				\fill[hfBlue!25] (0.45,0) circle (0.24);
				\fill[hfInk] (-0.45,0) circle (1.5pt);
				\fill[hfInk] (0.45,0) circle (1.5pt);
			\end{tikzpicture}
		};

		\node[panel] (b) at (-1.4,1.25) {
			\red{$\rho^\beta(\br)$}\\[0.4em]
			\begin{tikzpicture}
				\fill[hfRed!25] (-0.45,0) circle (0.24);
				\fill[hfRed!65] (0.45,0) circle (0.42);
				\fill[hfInk] (-0.45,0) circle (1.5pt);
				\fill[hfInk] (0.45,0) circle (1.5pt);
			\end{tikzpicture}
		};

		\node[panel] (total) at (2.2,2.25) {
			\green{$\rho^\text{T}(\br)$}\\
			$\rho^\alpha+\rho^\beta$\\[0.4em]
			\begin{tikzpicture}
				\fill[hfGreen!45] (-0.45,0) circle (0.40);
				\fill[hfGreen!45] (0.45,0) circle (0.40);
				\fill[hfInk] (-0.45,0) circle (1.5pt);
				\fill[hfInk] (0.45,0) circle (1.5pt);
			\end{tikzpicture}
		...
		\node[panel] (spin) at (2.2,-0.75) {
			\violet{$\rho^\text{S}(\br)$}\\
			$\rho^\alpha-\rho^\beta$\\[0.4em]
			\begin{tikzpicture}
				\fill[hfBlue!65] (-0.45,0) circle (0.38);
				\fill[hfRed!65]  (0.45,0) circle (0.38);
				\node[white] at (-0.45,0) {$+$};
				\node[white] at (0.45,0) {$-$};
			\end{tikzpicture}
		};

		\draw[->,very thick,hfGreen]
			(a.east) -- ++(0.35,0) |- (total.west);
		\draw[->,very thick,hfGreen]
			(b.east) -- ++(0.45,0) |- (total.west);

		\draw[->,very thick,hfViolet]
			(a.east) -- ++(0.35,0) |- (spin.west);
		\draw[->,very thick,hfViolet]
			(b.east) -- ++(0.45,0) |- (spin.west);
	\end{tikzpicture}

	\begin{block}{Density matrices}
		\begin{equation}
			\boxed{
			...
			\qquad
			\boxed{
				\bP^\text{S}=\bP^\alpha-\bP^\beta
			}
		\end{equation}
		The total density describes the charge distribution;
		the spin density reveals where the unpaired electrons reside.
	\end{block}
\end{frame}

\begin{frame}{GHF allows arbitrary spin directions}
	\centering
	\begin{tikzpicture}[>=stealth,scale=0.92]
		% RHF
		\draw[hfInk,thick] (-4.2,1.0) circle (1.15);
		\draw[hfInk!50] (-5.35,1.0) arc (180:360:1.15 and 0.32);
		\draw[hfInk!50,dashed] (-5.35,1.0) arc (180:0:1.15 and 0.32);
		\draw[->,very thick,hfBlue] (-4.2,1.0) -- (-4.2,1.9);
		\draw[->,very thick,hfRed]  (-4.2,1.0) -- (-4.2,0.1);
		\node[above] at (-4.2,2.35) {\textbf{RHF}};
		\node[align=center] at (-4.2,-0.65)
			{paired $\alpha/\beta$\\same spatial orbital};

		% UHF
		\draw[hfInk,thick] (0,1.0) circle (1.15);
		\draw[hfInk!50] (-1.15,1.0) arc (180:360:1.15 and 0.32);
		\draw[hfInk!50,dashed] (-1.15,1.0) arc (180:0:1.15 and 0.32);
		\draw[->,very thick,hfBlue] (0,1.0) -- (0,1.9);
		\node[above,text=hfBlue] at (0,1.9) {$+z$};
		\node[below,text=hfRed] at (0,0.1) {$-z$};
		\node[above] at (0,2.35) {\textbf{UHF}};
		\node[align=center] at (0,-0.65)
			{spin restricted to\\the common $z$ axis};

		% GHF
		\draw[hfInk,thick] (4.2,1.0) circle (1.15);
		\draw[hfInk!50] (3.05,1.0) arc (180:360:1.15 and 0.32);
		\draw[hfInk!50,dashed] (3.05,1.0) arc (180:0:1.15 and 0.32);
		\draw[->,very thick,hfViolet]
			(4.2,1.0) -- (4.95,1.68);
		\node[above] at (4.2,2.35) {\textbf{GHF}};
		\node[align=center] at (4.2,-0.65)
			{each orbital may choose\\its own spin direction};
	\end{tikzpicture}

	\begin{block}{Two-component spinor}
		\begin{equation}
			\boxed{
				\chi_p^\text{GHF}(\bx)
				=
	...
	\begin{center}
		\alert{GHF does not require a common spin-quantization axis.}
	\end{center}
\end{frame}

\begin{frame}{Born--Oppenheimer: electrons follow the nuclei}
	\begin{columns}[T]
		\begin{column}{0.50\textwidth}
			\begin{block}{Separation of time scales}
				\centering
				\begin{tikzpicture}[>=stealth,scale=0.9]
					% Nuclei
					\fill[hfOrange] (-1.7,0) circle (8pt);
					\fill[hfOrange] (1.7,0) circle (8pt);
					\node[below] at (-1.7,-0.25) {nucleus};
					\node[below] at (1.7,-0.25) {nucleus};

					% Electrons
					\fill[hfBlue] (-0.4,0.65) circle (3pt);
					\fill[hfBlue] (0.55,-0.55) circle (3pt);
					\fill[hfBlue] (0.9,0.75) circle (3pt);

					\draw[->,hfBlue,thick]
						(-0.8,0.9) arc (150:-110:0.55);
					\draw[->,hfBlue,thick]
						(0.1,-0.75) arc (210:520:0.55);

					\draw[<->,very thick,hfOrange]
						(-1.45,0) -- (1.45,0)
						node[midway,above] {$R$};

					\node[text=hfBlue,align=center] at (0,1.75)
						{fast electronic\\motion};
					\node[text=hfOrange,align=center] at (0,-1.45)
						{slow nuclear\\motion};
				\end{tikzpicture}
			\end{block}
		\end{column}
		%
		\begin{column}{0.50\textwidth}
			\begin{block}{Potential-energy surface}
				\centering
				\begin{tikzpicture}[scale=0.8,>=stealth]
					...
					\draw[dashed,hfRed]
						(1.75,0) -- (1.75,0.45);
					\node[below] at (1.75,0) {$R_\text{eq}$};
					\node[above right,text=hfBlue]
						at (2.45,0.72) {$E_\text{elec}(R)$};
				\end{tikzpicture}
			\end{block}
		\end{column}
	\end{columns}
	...
	\begin{center}
		For each fixed nuclear geometry $\bR$, solve the electronic problem;
		the resulting $E_k(\bR)$ governs the nuclear motion.
	\end{center}
\end{frame}

\begin{frame}{The Fock operator describes one electron in a mean field}
	\begin{columns}[T]
		\begin{column}{0.52\textwidth}
			\begin{block}{Physical picture}
				\centering
				\begin{tikzpicture}[>=stealth,scale=0.92]
					% Mean-field density
					\fill[hfBlue!15]
						(0,0) ellipse (3.0 and 1.5);
					\fill[hfBlue!16]
						(-1.1,0.2) ellipse (1.45 and 0.9);
					\fill[hfBlue!16]
						(1.15,-0.15) ellipse (1.5 and 0.95);

					% Nuclei
					\fill[hfOrange] (-1.3,0) circle (5pt);
					\fill[hfOrange] (1.3,0) circle (5pt);
					\node[below] at (-1.3,-0.2) {$A$};
					\node[below] at (1.3,-0.2) {$B$};

					% Probe electron
					\fill[hfRed] (0.35,1.5) circle (4pt);
					\node[above,text=hfRed] at (0.35,1.6)
						{electron $i$};

					\draw[->,thick,hfOrange]
						(0.25,1.35) -- (-1.1,0.2)
						node[midway,left] {nuclear attraction};

					\draw[->,thick,hfBlue]
						(0.45,1.35) -- (1.1,0.15)
						node[midway,right,align=left]
						{average\\electron field};

					\node[text=hfBlue] at (0,-1.8)
						{density of the other electrons};
				\end{tikzpicture}
			\end{block}
		\end{column}
		%
		\begin{column}{0.48\textwidth}
			\begin{block}{Effective one-electron operator}
				\begin{equation}
					\boxed{
						\hf(\bx)
						=
				...
				\end{equation}
				\begin{itemize}
					\item $\hh$: kinetic energy and nuclear attraction
					\item $\hJ_j$: classical Coulomb repulsion
					\item $\hK_j$: nonlocal exchange contribution
				\end{itemize}
			\end{block}

			\begin{block}{Self-consistent mean field}
				The potential depends on the occupied orbitals,
				which are themselves eigenfunctions of $\hf$:
				\begin{equation}
					\hf\chi_i=\epsilon_i\chi_i.
				\end{equation}
			\end{block}
		\end{column}
	\end{columns}
	...
	\begin{center}
		\alert{Each electron moves in the average field generated
		by all the others.}
	\end{center}
\end{frame}
