	...
	\begin{columns}[T]
		\begin{column}{0.49\textwidth}
			\begin{block}{Foundations}
				\begin{enumerate}
					\item \textbf{\blue{The electronic problem}}
					\\
					Molecular Hamiltonian, Born-Oppenheimer approximation,
					and Pauli principle
					\medskip

					\item \textbf{\orange{Hartree-Fock approximation}}
					\\
					Slater determinants, Hartree--Fock energy,
					Coulomb and exchange operators
					\medskip

					\item \textbf{\violet{Determinants and spin}}
					\\
					Excited determinants, Slater-Condon rules,
					spin adaptation and spin contamination
				\end{enumerate}
			\end{block}
		\end{column}
		%
		\begin{column}{0.49\textwidth}
			\begin{block}{Equations and solutions}
				\begin{enumerate}
					\setcounter{enumi}{3}

					\item \textbf{\blue{Finite-basis Hartree-Fock}}
					\\
					Fock operator, orbital energies,
					Roothaan--Hall equations and Koopmans' theorem
					\medskip

					\item \textbf{\orange{Hartree-Fock in practice}}
					\\
					SCF convergence, DIIS, excited-state SCF,
					properties and orbital localization
					\medskip

					\item \textbf{\violet{Beyond closed-shell RHF}}
					\\
					UHF, symmetry breaking in \ce{H2},
					the Coulson--Fischer point and GHF
				\end{enumerate}
			\end{block}
		\end{column}
	\end{columns}
...
\begin{frame}{Bosons vs Fermions}
	\begin{block}{Symmetry vs Antisymmetry}
		...
		\begin{equation*}
			\boxed{\abs{\Psi(\red{\bx_1},\blue{\bx_2})}^2 = \abs{\Psi(\blue{\bx_2},\red{\bx_1})}^2  
			\qq{$\Rightarrow$}  
			\Psi(\blue{\bx_2},\red{\bx_1}) = \green{e^{\ii \theta}} \Psi(\red{\bx_1},\blue{\bx_2})}
		\end{equation*}
		Applying twice the \alert{permutation operator} $\chP_{12}$ gives
		\begin{gather*}
			\chP_{12} \Psi(\red{\bx_1},\blue{\bx_2}) = \Psi(\blue{\bx_2},\red{\bx_1}) = \green{e^{\ii \theta}} \Psi(\red{\bx_1},\blue{\bx_2})
			\qquad
			\chP_{12}^2 \Psi(\red{\bx_1},\blue{\bx_2}) = \green{e^{2\ii \theta}} \Psi(\red{\bx_1},\blue{\bx_2}) = \Psi(\red{\bx_1},\blue{\bx_2})
			\\
			\qq{$\Rightarrow$}
			\green{e^{2\ii \theta}} = 1 
			\qq{$\Rightarrow$}
...
%-----------------------------------------------------
\begin{frame}{Notations for one-electron integrals and electron repulsion integrals (ERIs)}
	\begin{block}{Dirac or Physicists' notations}
		\begin{equation*}
			\mel{i}{\hh}{j} = h_{ij} = \mel{\chi_i}{\hh}{\chi_j} = \int \chi_i^*(\bx_1) \hh(\bx_1) \chi_j(\bx_1) \dd{\bx_1} 
		\end{equation*}
		\begin{equation*}
			\braket{ij}{kl} 
		...
		\begin{equation*}
			...
			= \iint \chi_i^*(\bx_1) \chi_j^*(\bx_2) \frac{1}{r_{12}} \qty(1 - \chP_{12}) \chi_k(\bx_1) \chi_l(\bx_2) \dd{\bx_1} \dd{\bx_2} 
		\end{equation*}
	\end{block}
	\begin{block}{Mulliken or Chemists' notations}
		\begin{equation*}
			\rmel{i}{\hh}{j} = h_{ij} = \rmel{\chi_i}{\hh}{\chi_j} = \int \chi_i^*(\bx_1) \hh(\bx_1) \chi_j(\bx_1) \dd{\bx_1} 
		\end{equation*}
		\begin{equation*}
			\rbraket{ij}{kl} 
	...
	\vfill
	\center
	\alert{Warning!} We will use the same notation for spin and spatial orbitals!
\end{frame}
...
\begin{frame}{Permutation symmetry}
	\begin{block}{Permutation symmetry in physicists' notation}
		...
		\begin{align*}
			...
			\braket{ij}{kl} & = \braket{ji}{lk} = \braket{kl}{ij} = \braket{lk}{ji} = \braket{kj}{il} = \braket{jk}{li} = \braket{il}{kj} = \braket{li}{jk}		
		\end{align*}
	\end{block}
	...
	\vfill
	\center
	For real-valued orbitals, the ERIs possess \alert{8-fold permutation symmetry}!
\end{frame}
...
\begin{frame}{Canonical Hartree-Fock equations}
	...
	\begin{block}{Canonical HF equations}
    	...
    	\begin{equation*}
    		\boxed{
    		\hf(\bx) \chi_i(\bx) = \epsilon_{i} \chi_i(\bx)
    		}
    	\end{equation*}
    	\begin{itemize}
...
\begin{frame}{DIIS algorithm}
	...
	\begin{tikzpicture}[
		...
		\node[scfstep,right=of mol] (ints)
			{Compute\\integrals};
		\node[scfstep,right=of ints] (guess)
		...
		\node[decision,left=of density] (conv)
			{Converged?};
		\node[scfstep,left=of conv] (properties)
...
\begin{frame}{DIIS take-home messages}
	\begin{center}
		\fbox{\textbf{Key idea:} DIIS accelerates SCF convergence by finding a linear combination of Fock matrices}
	\end{center}
	\bigskip
	\begin{itemize}
...
\begin{frame}{Localized orbitals}
	\only<1>{\begin{equation*}
		\bff = \mqty( \bff_\text{oo} & \bff_\text{ov} \\  \bff_\text{vo} &  \bff_\text{vv} )
		\qq{$\xrightarrow{\text{HF convergence}}$}
		\mqty( \bff_\text{oo} & \bO \\  \bO &  \bff_\text{vv} )
		\qq{$\xrightarrow{\text{canonicalization}}$}
		\mqty( \bepsilon_\text{o} & \bO \\  \bO &  \bepsilon_\text{v} )
		\qq{$\xrightarrow{\text{localization}}$}
		\mqty( \widetilde{\bff}_\text{oo} & \bO \\  \bO &  \widetilde{\bff}_\text{vv} )
	\end{equation*}}
...
\begin{frame}{GHF allows arbitrary spin directions}
	...
	\begin{tikzpicture}[>=stealth,scale=0.84]
		...
		\begin{scope}[shift={(-5.7,1.0)}]
			...
			% z axis
			\draw[->,hfInk!50] (0,-1.18) -- (0,1.18);
			\node[right,hfInk!60] at (0,1.2) {$z$};

			% alpha / beta
			\draw[->,very thick,hfBlue] (0,0) -- (0,0.80);
			\draw[->,very thick,hfRed]  (0,0) -- (0,-0.80);

			\node[right,text=hfBlue] at (0.05,0.62) {$\alpha$};
			\node[right,text=hfRed]  at (0.05,-0.62) {$\beta$};
		\end{scope}

		\node[above] at (-5.7,2.20) {\textbf{RHF}};
		\node[align=center] at (-5.7,-0.55)
			{common $z$ axis\\
			 \alert{same spatial orbital}};


		% ============================================================
		% UHF
		% ============================================================
		\begin{scope}[shift={(-1.9,1.0)}]
			% Bloch sphere
			\draw[hfInk,thick] (0,0) circle (1.00);
			\draw[hfInk!50] (-1.00,0) arc (180:360:1.00 and 0.28);
			\draw[hfInk!50,dashed] (-1.00,0) arc (180:0:1.00 and 0.28);
		...
		\node[above] at (-1.9,2.20) {\textbf{UHF}};
		\node[align=center] at (-1.9,-0.55)
			{common $z$ axis\\
			 \alert{different spatial orbitals}};
...
\begin{scope}[shift={(1.9,1.0)}]
	...
	% laboratory z axis
	\draw[->,hfInk!35] (0,-1.18) -- (0,1.18);
	\node[right,hfInk!50] at (0,1.2) {$z$};

	% antiparallel spin directions
	\draw[->,very thick,hfViolet]
		(0,0) -- (0.66,0.58);
	\draw[->,very thick,hfOrange]
		(0,0) -- (-0.66,-0.58);

	% labels outside the sphere
	\node[above right,text=hfViolet]
		at (0.0,0.4) {$\boldsymbol{s}$};
	\node[below left,text=hfOrange]
		at (0.0,-0.4) {$\boldsymbol{s}'$};
\end{scope}

\node[above] at (1.9,2.20) {\textbf{Rotated UHF}};
\node[align=center] at (1.9,-0.55)
	{spinors along\\
	 \alert{a common tilted axis}};
...
\begin{scope}[shift={(5.7,1.0)}]
	...
	\node[above right,text=hfViolet]
		at (0.0,0.4) {$\boldsymbol{s}$};
	\node[above left,text=hfOrange]
		at (-0.3,0.1) {$\boldsymbol{s}'$};
\end{scope}

\node[above] at (5.7,2.20) {\textbf{GHF}};
\node[align=center] at (5.7,-0.55)
	{spinors along\\
	 \alert{different tilted axes}};

	\end{tikzpicture}

	\vfill

	\begin{block}{Collinear vs non-collinear solutions}
		\begin{equation}
			\underbrace{
				\text{UHF}
				\xrightarrow{\text{global spin rotation}}
				\text{rotated UHF}
			}_{\text{physically equivalent collinear solutions}}
			\qquad\neq\qquad
			\underbrace{
				\text{genuine GHF}
			}_{\text{non-collinear}}
	...
	\centering
	$\Rightarrow$
	\alert{A tilted spin axis alone does not imply a genuinely non-collinear GHF solution}
\end{frame}
