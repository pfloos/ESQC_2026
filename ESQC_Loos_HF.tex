...
\begin{frame}{Maximum overlap method (MOM)}
	Let $\bC_{\text{o}}^\text{ref}$ contain the occupied spin orbitals defining
	the target configuration. For each new spin orbital $p$, compute
	\begin{equation}
		O_{ip}
		=
	...
	\begin{columns}[T,onlytextwidth]
		...
		\begin{column}{0.48\textwidth}
			\centering
			\textbf{Projection onto the reference space}
			...
			\begin{tikzpicture}[
				...
				{
					occupied reference space\\[-1mm]
					$\mathcal O_{\mathrm{ref}}
					=\operatorname{span}
					\{\chi_i^\mathrm{ref}\}$
				};
				...
				\node[hfInk] at (2.0,3.80)
					{overlap with $\mathcal O_{\mathrm{ref}}$};
		...
		\begin{column}{0.47\textwidth}
			\begin{block}{What is gained?}
				\begin{itemize}
					...
					\bigskip
					\item Accurate excitation energies for states dominated by a single electronic configuration
					\bigskip
