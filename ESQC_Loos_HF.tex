...
\begin{frame}{The Fock matrix}
	The \alert{stationarity of the HF energy} with respect to the spin orbitals implies that 
	\begin{equation}
		\fdv{E_\text{HF}}{\chi_i} = 0
		\qq{$\Rightarrow$}
		\boxed{
		\hf(\bx_1) \chi_i(\bx_1) = \underbrace{\epsilon_i}_{\alert{\text{orbital energy}}} \underbrace{\chi_i(\bx_1)}_{\blue{\text{canonical orbital}}}
		}
	\end{equation}	
...
\begin{frame}{Koopmans prefers IPs}
	...
	\begin{split}
		...
		\\
		& = E_\text{HF}^{N-1} - E_\text{HF}^{N} + E_\text{c}^{N-1} - E_\text{c}^{N}
		\\
		& = - \epsilon_i + \underbrace{\Delta_\text{relax}}_{\text{relaxation}} + \underbrace{\Delta_\text{corr}}_{\text{correlation}}
	\end{split}
	\end{equation}
	For IPs, there is large error compensation!
	It does not apply to EAs.
\end{frame}
		...
		\begin{column}{0.48\textwidth}
			\begin{block}{What remains unchanged}
				\begin{itemize}
    				...
    				\begin{equation}
    					\widetilde{\boldsymbol{f}}_\text{ov} = \boldsymbol{f}_\text{ov}  = \bO
    				\end{equation}
				\end{itemize}
			\end{block}
		...
		\begin{column}{0.48\textwidth}
			\begin{block}{What changes}
				\begin{itemize}
    				...
    				\begin{equation}
    					\widetilde{\boldsymbol{f}}_\text{oo}
    					=
