...
%				\node[hfRed,left] at (-0.75,0.72) {hole};
	...
	\begin{block}{The problem}
		...
		\\
		$\Rightarrow$ the SCF tends to return to a lower-energy solution.
	\end{block}
\end{frame}
...
\begin{frame}{Maximum overlap method (MOM)}
	Let $\bC_{\text{o}}^\text{ref}$ contain the occupied orbitals that define
	the target configuration. For each new orbital $p$, compute
	\begin{align}
		O_{ip}
		&=
		\braket{\psi_i^\text{ref}}{\psi_p^\text{new}}
		=
		\sum_{\mu\nu}
		...
		\kappa_p
		&=
		1 - \sum_{i\in\text{ref}}\abs{O_{ip}}^2
	\end{align}
	...
	\begin{columns}[T,onlytextwidth]
		\begin{column}{0.50\textwidth}
			\begin{block}{MOM procedure}
				\begin{itemize}
					\item Rank the new orbitals using $\kappa_p$
					\item Occupy the $N$ orbitals with the smallest $\kappa_p$
					\item In MOM, the reference is the occupied space from the previous SCF iteration
					\item Initial MOM (IMOM): The reference orbitals remain fixed to the
...
\begin{frame}{MOM-based SCF}
	...
	\begin{tikzpicture}[
		...
		\node[box,right=of solve] (overlap) {Compute $\kappa_p$};
		\node[box,right=of overlap] (occupy) {Occupy largest\\$\kappa_p$ values};
	...
	\begin{columns}[T,onlytextwidth]
		...
		\begin{column}{0.47\textwidth}
			\begin{block}{MOM-SCF}
				\centering
				Occupy the smallest $\kappa$ values
				\begin{equation}
					\kappa_1\leq\kappa_2\leq\cdots\leq\kappa_N
				\end{equation}
