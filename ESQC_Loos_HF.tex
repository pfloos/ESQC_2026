...
\begin{frame}{Spin contamination in UHF}
	...
	\begin{equation}
		\boxed{
			...
			=
			\underbrace{\expval{\purple{\chS^2}}_{\text{exact}}}_{S(S+1)}
			+
			N^\beta
			-
			\sum_{i=1}^{N^\alpha}
...
%-----------------------------------------------------
\begin{frame}{Are RHF excited determinants spin eigenfunctions?}
	...
	\center
	$\Rightarrow$
	\alert{RHF open-shell determinants might not be pure spin functions!}
\end{frame}
...
\begin{frame}{Normalization of a two-electron determinant}
	\begin{block}{Problem: Normalization of the HF wavefunction}
		\textit{\violet{``Show that the HF wavefunction built with two (normalized) spin orbitals $\chi_1$ and $\chi_2$ is normalized''}}
	\end{block}
	\pause
	\begin{block}{Solution}
		\small
...
\begin{frame}{DIIS take-home messages}
	\begin{itemize}
		...
		\bigskip
		\item $\bB$ can be \alert{singular} due to nearly linear dependence of the residuals: restart possible
		\bigskip
...
\begin{frame}{Localization procedures}
	\begin{block}{Edmiston-Ruedenberg functional}
    	...
    	\end{equation}
	\end{block}	
	\begin{block}{Boys-Foster functional}
    	\begin{equation}
    	\begin{split}
    			\Omega_\text{BF}
    			& =
...
%-----------------------------------------------------
\begin{frame}{The dissociation curves follow from the asymptotic wavefunctions}
	\begin{columns}[T,onlytextwidth]
...
\begin{frame}{GHF equations: Roothaan-Hall in spinor space}
	...
	\begin{block}{Fock matrix}
    	...
    	\begin{align}
    		J[\bP]_{\mu\nu}
    		&=\sum_{\lambda\sigma}P_{\lambda\kappa}\rbraket{\mu\nu}{\lambda\sigma}
    		&
    		K[\bP]_{\mu\nu}
    		&=\sum_{\lambda\sigma}P_{\lambda\kappa}\rbraket{\mu\sigma}{\lambda\nu}
    	\end{align}
...
\begin{frame}{Some comments on GHF}
	...
	\begin{block}{Take-home messages}
		\begin{itemize}
			...
			\item One can break the particle number within \alert{Hartree-Fock-Bogoliubov (HFB)} theory
		\end{itemize}
	\end{block}
\end{frame}
%-----------------------------------------------------
