...
\begin{frame}{Some remarks}
	\begin{itemize}
		\item Because these excited determinants are built with ground-state orbitals, they are not genuine HF states.
		\item They do not correspond to stationary points of the HF equations.
		\item They might not even be eigenfunctions of the spin operator
	\end{itemize}
\end{frame}

\begin{frame}{Spin-adapted determinants}
	In the nonrelativistic framework, $\chH$ commutes with $\chS^2$ and $\chS_z$ 
	\begin{equation}
		\comm{\chH}{\chS^2} = 0 = \comm{\chH}{\chS_z}
	\end{equation}
	They share a common set of eigenfunctions!
	\begin{equation}
		\chS_z \ket{\Phi} = M_S \ket{\Phi}
		\chS^2 \ket{\Phi} = S(S+1) \ket{\Phi}
	\end{equation}
	\begin{equation}
		\chS_z \ket{\cdots \psi_i \psi_j \cdots} = \frac{N^\alpha - N^\beta}{2} \ket{\cdots \psi_i \psi_j \cdots}
	\end{equation}
	RHF and UHF determinants are always eigenfunctions of $\chS_z$!
	\begin{equation}
		\chS^2 \ket{\cdots \psi_i \Bar{\psi}_i \psi_j \Bar{\psi}_j \cdots} = 0(0+1) \ket{\cdots \psi_i \Bar{\psi}_i  \psi_j \Bar{\psi}_j \cdots}
	\end{equation}
	Closed-shell determinants are pure singlets!
	
\end{frame}

\begin{frame}{Spin contamination in UHF}
	\begin{itemize}
		\item UHF wave functions are not eigenfunctions of $\chS^2$
	\begin{equation}
		\expval{\chS^2}_\text{UHF} = \expval{\chS^2}_\text{exact} + N^\beta - \sum_{ij} \abs{S_{ij}^{\alpha\beta}}^2
		\qq{with}
		\expval{\chS^2}_\text{exact} = \frac{N^\alpha - N^\beta}{2} \qty( \frac{N^\alpha - N^\beta}{2} + 1 )
	\end{equation}
		\item They cannot be spin-adapted with a small number of determinants
		\item UHF wave functions are contaminated by higher, not lower, multiplicity components
