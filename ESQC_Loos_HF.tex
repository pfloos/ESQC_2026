...
\begin{frame}{Spin of many-electron wavefunctions}
	\begin{block}{Spin operators}
    	...
    	\end{equation}  
    	The eigenfunctions of $\chH$ may therefore be chosen as simultaneous eigenfunctions of $\chS^2$ and $\chS_z$:
    	\begin{align}
    		\chS^2\ket{S,M_S}
    		& =
    		S(S+1)\ket{S,M_S}
	...
	\begin{block}{Spin quantum numbers}
		\begin{itemize}
			\item $S$ determines the \alert{spin multiplicity} $2S+1$: 
			\begin{itemize}
				...
				\item \ldots
			\end{itemize}
			\item $M_S=-S,-S+1,\ldots,S-1,S$ is its \alert{spin projection}
		\end{itemize}
	\end{block}
\end{frame}
...
\begin{frame}{Are excited determinants genuine spin functions?}
	For a \alert{collinear$^*$} determinant containing $N^\alpha$ spin-up and $N^\beta$ spin-down orbitals, one has
	\begin{equation}
		\boxed{
			\chS_z\ket{\Phi} = M_S\ket{\Phi}
			\qq{with}
	...
	\bigskip
	\footnotesize
$^*$In a collinear determinant, the occupied spin orbitals all have spin aligned along one common quantization axis, conventionally $z$
\end{frame}
...
\begin{frame}{Diagonalizing $\chS^2$ produces pure singlet and triplet states}
	...
	\vfill
	\center
	$\Rightarrow$ \alert{spin adaptation = diagonalization of $\chS^2$ in a determinant space}
\end{frame}
...
\begin{frame}{The three components of the triplet}
	The three components of the $S=1$ multiplet are
		\begin{align}
			...
			\ket{{}^{3,0}\Psi_i^a}
				&=\frac{1}{\sqrt2}
				\left(\ket*{D_\alpha}-\ket*{D_\beta}\right)
				&
			\ket{{}^{3,-1}\Psi_i^a}
