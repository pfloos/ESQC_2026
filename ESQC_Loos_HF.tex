...
\begin{frame}{RHF problem}
	The closed-shell RHF wavefunction doubly occupies the bonding orbital:
	\begin{equation}
		\ket{\Psi_\text{RHF}}
		=
	...
	\begin{columns}[T,onlytextwidth]
		...
		\begin{column}{0.48\textwidth}
			\begin{block}{Large bond length}
				When $0 \le \cos^2 \theta^\star \le 1$, two minima appear at $\pm\theta^\star$, and RHF becomes
...
\begin{frame}{Canonical occupied orbitals are not unique}
	Let $\bC_\text{occ}$ contain the occupied canonical orbitals. Any unitary
	rotation within the occupied space defines an equivalent set:
	\begin{equation}
		\boxed{
			\widetilde{\bC}_\text{occ}
			=
			\bC_\text{occ}\cdot\bU
			\qquad
		...
		\begin{column}{0.48\textwidth}
			\begin{block}{The density matrix is unchanged}
				...
				\begin{equation}
					\widetilde{\bP}
					=
					2\widetilde{\bC}_\text{occ}
					\cdot
					\widetilde{\bC}_\text{occ}^\dagger
					=
					2\bC_\text{occ}
					\cdot
					\bC_\text{occ}^\dagger
