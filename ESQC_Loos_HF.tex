...
\begin{frame}{How do I know I have converged (or not)?}
	...
	\begin{enumerate}
		...
		\item 	At self-consistency, the density and Fock matrices commute:
    	\begin{equation}
    		\boxed{
    			\be^{(n)}
    			=
		...
		$\Rightarrow$ \alert{Extrapolation of the Fock matrix} using the $m$ previous iterations
		\begin{equation}
...
\begin{frame}{DIIS take-home messages}
	\begin{itemize}
		\item DIIS accelerates SCF convergence by finding a linear combination of previous Fock matrices whose extrapolated residual is minimal
		\item A small rolling subspace is normally sufficient: commonly $m\approx6$--$10$
		\item DIIS requires additional storage: $\sim 2mK^2$
		\item The extrapolated energy is not guaranteed to decrease monotonically
		\item $\bB$ can be singular due to nearly linear dependence of the residuals: restart possible
