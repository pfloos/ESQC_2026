...
\paragraph{Worked example: two-electron antisymmetry.}
...
\paragraph{Worked example: normalization of the HF wavefunction.}
...
\paragraph{Worked example: core Hamiltonian.}
...
\paragraph{Worked example: two-electron Hamiltonian.}
...
\paragraph{Worked example: three-electron system.}
...
\paragraph{Worked example: HF energy of the \ce{Li} atom.}
...
\paragraph{Worked example: HF energy of the \ce{B} atom.}
...
An excited determinant is an orbital substitution within a \red{fixed} set of
...
\red{spin projection} or \red{a linear combination of determinants}.
...
Minimizing the Hartree--Fock energy under the orbital orthonormality
constraints produces an effective one-electron operator. Its eigenfunctions
...
\paragraph{Worked example: Fock matrix elements in the spin-orbital basis.}
...
\paragraph{Worked example: orbital energies in the spin-orbital basis.}
...
\paragraph{Worked example: Koopmans' theorem for electron affinity.}
...
\paragraph{Worked example: AO Fock matrix.}
...
\begin{equation}
  \chi_i
  =
  \sum_\lambda C_{\lambda i}\phi_\lambda,
  \qquad
  \chi_i^*
  =
  \sum_\sigma C_{\sigma i}^*\phi_\sigma^*.
\end{equation}
...
\paragraph{Worked example: AO Hartree--Fock energy.}
...
spatial orbital. Correspondingly, the exchange term in
...
\paragraph{Fixed quantities and iterative quantities.}
...
why the expensive integral generation is performed before the iterative cycle,
whereas the Fock build and diagonalization are repeated.

\paragraph{The SCF algorithm.}

In practice, a restricted Hartree--Fock calculation begins by
\red{specifying the molecule} through $\{\bR_A\}$ and $\{Z_A\}$ and choosing
the \red{basis set} $\{\phi_\mu\}$. The integrals $S_{\mu\nu}$,
$H_{\mu\nu}$, and $\braket{\mu\nu}{\lambda\sigma}$ are then calculated,
after which $\bS$ is diagonalized to obtain $\bX$.

Once an \red{initial density matrix} $\bP$ has been chosen, the iterative
...
$\bE$. The AO coefficients are recovered as
$\bC=\bX\cdot\bC'$, and a \red{new density matrix} is formed from
...
Several initial density matrices are possible. Setting \red{$\bP=\bO$, so
that $\bF=\bH$}, gives the \red{core guess}; it is usually poor, but it is
easy to implement. An inexpensive, though now less common, alternative is to
use \red{extended H\"uckel theory or a semi-empirical method}. A
superposition-of-atomic-densities (SAD) guess is obtained from
\red{tabulated atomic densities}. Finally, it is very common and useful to
\red{read the molecular orbitals from a previous calculation}.
...
In an orthonormal basis, the corresponding condition is simply that the Fock
...
A small \red{rolling subspace}, commonly
...
\red{storage} of approximately $2mK^2$. Moreover, the extrapolated energy is
not guaranteed to decrease monotonically, and $\bB$ can become
\red{singular} when the residuals are nearly linearly dependent, in which
...
spin orbitals with the \red{largest $w_p$}. In MOM, the reference occupied
space is taken from the previous iteration, whereas in \red{IMOM} the
reference orbitals remain fixed to the initial guess.
...
MOM provides a higher-energy stationary solution of the HF equations, with
full orbital relaxation for the targeted configuration and a corresponding
...
The electronic term follows from the one-particle density:
\begin{align}
...
The occupied subspace and Slater determinant remain unchanged, as do the
...
coupled Pople--Nesbet equations are developed before the unrestricted SCF
...
\paragraph{Worked example: UHF energy of the \ce{Li} atom.}
...
begins by \red{specifying the molecule} through $\{\bR_A\}$ and $\{Z_A\}$
and choosing the \red{basis set} $\{\phi_\mu\}$, as in RHF. The integrals
$S_{\mu\nu}$, $H_{\mu\nu}$, and
$\braket{\mu\nu}{\lambda\sigma}$ are calculated, and $\bS$ is diagonalized
to obtain $\bX$.

After \red{initial density matrices} $\bP^\alpha$ and $\bP^\beta$ have been
chosen, the two spin-dependent matrix builds are performed:
...
$(\bC^\alpha)'$ and $\bE^\alpha$, together with $(\bC^\beta)'$ and
$\bE^\beta$. The AO coefficients are then recovered from
$\bC^\alpha=\bX\cdot(\bC^\alpha)'$ and
$\bC^\beta=\bX\cdot(\bC^\beta)'$. These coefficients define the
\red{new density matrices} $\bP^\alpha$ and $\bP^\beta$, from which
...
\end{equation}
Because $\dd x/\dd\theta=-\sin(2\theta)$, stationarity requires either
$\sin(2\theta)=0$ or $\dd E_\text{UHF}/\dd x=0$. The first condition contains
...
\paragraph{Variational hierarchy and stability.}
...
Computational Chemistry}, Cramer's \emph{Essentials of Computational
Chemistry}, Szabo and Ostlund's \emph{Modern Quantum Chemistry}, and
