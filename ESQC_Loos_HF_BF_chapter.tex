...
$N$-electron wavefunction, we restrict it to a single Slater
...
This leads naturally to the mean-field character of Hartree--Fock
...
out a potential-energy surface (PES). Equilibrium structures correspond
...
states defines an electron-repulsion integral (ERI). In physicists'
notation,
\begin{equation}
  \braket{ij}{kl}
  =
...
Slater--Condon rules avoid this combinatorial expansion. They express
...
\end{equation}
This result is Brillouin's theorem.
...
The quantum number $S$ determines the spin multiplicity,
\begin{equation}
...
\begin{equation}
  \text{an open-shell determinant need not be a pure spin function.}
\end{equation}
...
Thus,
\begin{equation}
  \text{spin adaptation is the diagonalization of $\chS^2$
...
This is the content of Koopmans' theorem. Its central approximation is
...
\begin{equation}
  \text{How do we solve these HF equations?}
\end{equation}
...
the density matrix $\bP$. This dependence makes the Roothaan--Hall
...
read the molecular orbitals from a previous calculation.
...
\begin{equation}
  \bP^{(n+1)}=\bP^{(n)}=\bP^\star.
\end{equation}
One may check the energy and/or the density matrix. At
self-consistency, the generalized commutator vanishes:
\begin{equation}
    \be^{(n)}
...
space is taken from the previous iteration, whereas in IMOM the
...
\end{equation}
and therefore follows a chosen SCF solution. Thus, MOM changes the
...
\end{equation}
and the Coulson--Fischer point is defined by
\begin{equation}
    R=R_\text{CF}
    \quad\Longleftrightarrow\quad
...
within Hartree--Fock--Bogoliubov (HFB) theory.
